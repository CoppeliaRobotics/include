...
function(COPPELIASIM_GENERATE_STUBS GENERATED_OUTPUT_DIR)
    ...
    if(NOT "${COPPELIASIM_GENERATE_STUBS_LUA_FILE}" STREQUAL "")
        ...
        install(FILES ${GENERATED_OUTPUT_DIR}/typecheck.lua RENAME sim${PLUGIN_NAME_AND_VERSION}-typecheck.lua DESTINATION ${COPPELIASIM_LUA_DIR})
        install(FILES ${COPPELIASIM_GENERATE_STUBS_LUA_FILE} RENAME sim${PLUGIN_NAME_AND_VERSION}.lua DESTINATION ${COPPELIASIM_LUA_DIR})
    endif()
    add_custom_command(OUTPUT ${OUTPUT_FILES} COMMAND ${COMMAND_ARGS} DEPENDS ${DEPENDENCIES})
    set(REFERENCE_FILE_NAME sim${PLUGIN_NAME_AND_VERSION}.htm)
    set(INDEX_FILE_NAME sim${PLUGIN_NAME_AND_VERSION}.json)
    ...
    if(NOT COPPELIASIM_GENERATE_STUBS_OLD_STYLE)
        ...
        install(FILES ${GENERATED_OUTPUT_DIR}/ce.lua RENAME sim${PLUGIN_NAME_AND_VERSION}-ce.lua DESTINATION ${COPPELIASIM_LUA_DIR})
    endif()
...
function(COPPELIASIM_ADD_PLUGIN PLUGIN_TARGET_NAME)
    ...
    if(NOT COPPELIASIM_ADD_PLUGIN_LEGACY AND PLUGIN_TARGET_NAME MATCHES "^simExt")
        target_compile_definitions(${PLUGIN_TARGET_NAME_V} PRIVATE SIM_PLUGIN_OLD_ENTRYPOINTS)
    endif()
...
function(COPPELIASIM_ADD_HELPFILE HELP_FILE)
    ...
    if(COPPELIASIM_ADD_HELPFILE_SUBDIR)
        install(FILES ${HELP_FILE} DESTINATION ${COPPELIASIM_HELPFILES_DIR}/${COPPELIASIM_ADD_HELPFILE_SUBDIR})
    else()
        install(FILES ${HELP_FILE} DESTINATION ${COPPELIASIM_HELPFILES_DIR}/en)
    endif()
endfunction(COPPELIASIM_ADD_HELPFILE)

function(COPPELIASIM_ADD_HELPFILE_DIRECTORY HELP_DIR)
    cmake_parse_arguments(COPPELIASIM_ADD_HELPFILE_DIRECTORY "" "SUBDIR" "" ${ARGN})
    if(COPPELIASIM_ADD_HELPFILE_DIRECTORY_SUBDIR)
        install(DIRECTORY ${HELP_DIR} DESTINATION ${COPPELIASIM_HELPFILES_DIR}/${COPPELIASIM_ADD_HELPFILE_DIRECTORY_SUBDIR})
    else()
        install(DIRECTORY ${HELP_DIR} DESTINATION ${COPPELIASIM_HELPFILES_DIR})
    endif()
endfunction(COPPELIASIM_ADD_HELPFILE_DIRECTORY)
