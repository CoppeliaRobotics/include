...
parser.add_argument('--xml-file', type=str, default=None, help='the callbacks.xml file')
parser.add_argument('--lua-file', action='append', type=str, default=None, help='the lua file')
parser.add_argument('--xml-output-file', type=str, default=None, help='the output lua.xml file')
...
for lua_file in args.lua_file or []:
    def error(msg):
        global args, lineno
        print(f'{lua_file}:{lineno}: {msg}')
        sys.exit(2)
    ...
    with open(lua_file, 'r') as f:
        for lineno, line in enumerate(f):
            ...
            try:
                ...
                raise Exception(f'error at {lua_file}:{lineno}: {e!s}')
        output()

tree = ET.ElementTree(root)
if not args.dry_run:
    tree.write(args.xml_output_file, encoding='utf-8', xml_declaration=True)
if args.dry_run:
